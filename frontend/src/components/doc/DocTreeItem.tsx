...
export const DocTreeItem = ({
  ...
  // Owner + Member can create/edit/archive; Viewer is read-only
  ...
  const handleAddSubDoc = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!canEdit) {
      showToast('You do not have permission to add sub-pages', 'error');
      return;
    }
    ...
    try {
      ...
      setIsExpanded(true);
      showToast('Sub-page created successfully', 'success');
      navigate(`/workspaces/${workspaceId}/docs/${newDoc._id}`);
    } catch (err: any) {
      showToast(err.message || 'Failed to create sub-document', 'error');
    }
  };
  ...
  const handleArchiveClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!canEdit) {
      showToast('You do not have permission to archive pages', 'error');
      return;
    }
  ...
  const handleConfirmArchive = async () => {
    ...
    try {
      ...
      showToast('Page archived successfully', 'success');
      setIsConfirmOpen(false);
      if (isActive) {
        navigate(`/workspaces/${workspaceId}/docs`);
      }
    } catch (err: any) {
      showToast(err.message || 'Failed to archive document', 'error');
    } finally {
  ...
  return (
    <div className="select-none">
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          <button
            ...
            className={`p-0.5 rounded hover:bg-slate-200 transition-transform cursor-pointer ${
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to archive "${node.title || 'Untitled'}"? This page will be hidden from the wiki.`}
        confirmText="Archive"
        variant="danger"
